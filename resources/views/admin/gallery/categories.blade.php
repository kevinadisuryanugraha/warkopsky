...
@section('title', 'Kelola Kategori Galeri | Warkop Sky CRM')
@section('page_title', 'Kelola Kategori Galeri')
...
@section('content')
    ...
    <div class="action-header-bar" style="justify-content: flex-end;">
        <a href="{{ route('admin.gallery.categories.create') }}" class="btn-add-menu">
                ...
                @foreach($categories as $category)
                    <tr>
                        ...
                        <td style="font-weight: 600; color: var(--color-warm-gold);">
                            {{ $category->items()->count() }} Foto
                        </td>
                        <td style="text-align: center;">
                            <form action="{{ route('admin.gallery.category.destroy', $category->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin mau menghapus kategori galeri {{ $category->name }}?')" style="display: inline-block;">
                                @csrf
                                @method('DELETE')
